"""
Generative Agents - Day 4
Adds daily plan generation + hourly time-stepped simulation.

What's new vs Day 3:
  - Each agent generates a 24-hour plan (location + activity per hour) at day start
  - Simulation advances hour-by-hour through a full day
  - When two agents share a location at the same hour, a conversation is triggered
  - End-of-day each agent reflects on the whole day

Data flow per hour:
  for agent in agents:
      1. check plan[hour] -> (location, activity)
      2. record observation "我在 {location} {activity}"
      3. if co-located with another agent -> trigger short conversation
  at day end:
      each agent runs reflect() with day-end prompt
    ...
    for hour in range(start_h, end_h + 1):
        ...
        for loc, occupants in loc_map.items():
            if len(occupants) >= 2 and random.random() < CONVERSATION_PROBABILITY:
                # Pick first two — good enough for 2-agent MVP
